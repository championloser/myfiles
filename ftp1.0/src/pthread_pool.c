...
void* pthfunc_ftp(void* p)//ftp线程入口函数
{
	...
	while(1)
	{
		...
		printf("%s:pthread %ld,%ld:ftp_work start\n",cur_time(),syscall(SYS_gettid),pthread_self());
		ftp_work_server(pfdno->fd,&pthinfo->cdmutex);//ftp服务器端功能函数
		printf("%s:pthread %ld,%ld:ftp_work finish\n",cur_time(),syscall(SYS_gettid),pthread_self());
		free(pfdno);
	}
}
void* pthfunc(void* p)//发文件的线程入口函数
{
	Pthnodepoint pthinfo=(Pthnodepoint)p;
	Fdnodepoint pfdno;
	int ret;
	while(1)
	{
		...
		ret=send_file(pfdno->fd,FILENAME,FILENAME);//发文件
		if(0==ret)
		{
...
int Init_pthinfo(Pthnodepoint pthinfo,int pthnum,Entryfunc pthfunc)
{
	...
	pthread_mutex_init(&pthinfo->cdmutex,NULL);
